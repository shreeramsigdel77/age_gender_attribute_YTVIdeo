...
while (capture.isOpened()):
    ...
    now = datetime.now()
    file_name_date =now.strftime("%Y-%m-%d")
    current_time = now.strftime("%H:%M:%S")
    current_dt = {
    ...
    if grabbed == True:
        ...
        if counter == 450:
            list_images, _ = ped_detectron(frame)
            # print(list_images)
            ...
            if len(list_images):
                ...
                if len(gender_dict):
                    ...
                    append_dict(f"/home/nabusri/BackupFiles/workspace/pedestrian_attribute_recog/resnet50_custom_pedestrian/result_log/result_{file_name_date}.jsonl",full_dict)
